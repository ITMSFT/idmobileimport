# 📄 Техническое задание на разработку модуля интеграции с сайтом (UA/RU)
...
1. **Основний YML-фід:** Містить повну інформацію про товарний каталог, категорії, характеристики та роздрібні ціни.

2. **CSV-файл оновлення:** Містить оперативну інформацію щодо залишків та гуртових цін для швидкої синхронізації.

Обидва файли повинні вивантажуватися в один і той самий каталог на SFTP-сервері та мати **статичні (незмінні) імена**.
...
* **Періодичність генерації:** Налаштовувана, за замовчуванням — 1 раз на добу.

#### 2.2. Ключові вимоги до структури та даних

##### 2.2.1. Дата генерації

Кожен файл повинен містити дату та час його створення в атрибуті `date`.
...
Необхідно вказати основну валюту — гривню (UAH).
...
Вивантажуються всі товарні категорії зі збереженням ієрархії. Назви категорій вивантажуються **лише українською мовою**.
...
##### 2.2.4. Товарні пропозиції (`<offer>`)

* **`id` (Унікальний ідентифікатор):** **Критично важливо** - має бути унікальним для кожної товарної номенклатури (SKU).

* **`group_id` (Ідентифікатор групи):** Використовується для об'єднання варіантів одного товару.

* **Мультимовність:** Мова за замовчуванням — українська (`<name>`, `<description>`). Додатково — російськомовні аналоги (`<name_ru>`, `<description_ru>`).

* **Ціни:** Роздрібна `<price>` та опціонально закупівельна `<purchase_price>`.

* **Характеристики (`<param>`):** Вивантажуються **ВИКЛЮЧНО українською мовою**.
...
* **Періодичність генерації:** Налаштовувана, за замовчуванням — кожні 15 хвилин.
...
* **Каталог для вивантаження:** `/path/to/import/` (уточнити у хостинг-провайдера)
...
1. **Основной YML-фид:** Содержит полную информацию о товарном каталоге, категориях, характеристиках и розничных ценах.

2. **CSV-файл обновления:** Содержит оперативную информацию по остаткам и оптовым ценам для быстрой синхронизации.

Оба файла должны выгружаться в один и тот же каталог на SFTP-сервере и иметь **статичные** (неизменяемые) **имена**.
...
* **Периодичность генерации:** Настраиваемая, по умолчанию — 1 раз в сутки.

#### 2.2. Ключевые требования к структуре и данным

##### 2.2.1. Дата генерации

Каждый файл должен содержать дату и время его создания в атрибуте `date`.
...
Необходимо указать основную валюту — гривну (UAH).
...
Выгружаются все товарные категории с сохранением иерархии. Названия категорий выгружаются **только на украинском языке**.
...
##### 2.2.4. Товарные предложения (`<offer>`)

* **`id` (Уникальный идентификатор):** **Критически важно** - должен быть уникальным для каждой товарной номенклатуры (SKU).

* `group_id` (Идентификатор **группы):** Используется для объединения вариантов одного товара.

* **Мультиязычность:** Язык по умолчанию — украинский (`<name>`, `<description>`). Дополнительно — русскоязычные аналоги (`<name_ru>`, `<description_ru>`).

* **Цены:** Розничная `<price>` и опционально закупочная `<purchase_price>`.

* **Характеристики (`<param>`):** Выгружаются **ИСКЛЮЧИТЕЛЬНО на украинском языке**.
...
* **Периодичность генерации:** Настраиваемая, по умолчанию — каждые 15 минут.
...
* **Каталог для выгрузки:** `/path/to/import/` (уточнить у хостинг-провайдера)
